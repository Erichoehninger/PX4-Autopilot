...
public:
	SensorCanRx(int32_t my_id) :
		ScheduledWorkItem("sensor_can_rx", px4::wq_configurations::lp_default),
		_my_id(my_id)
	{
		init_socket();
	...
	void Run() override
	{
		...
		while (true) { //drena o pipe
            ...
            if (rx.timestamp_utc > 0 && latency < 100000) { // descarta latência superior a 100 ms
